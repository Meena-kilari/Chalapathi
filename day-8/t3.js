...
/*function formValidation()
{
    var username1 = document.registration.usernamr;
    var age1 = document.registration.age;
    var pincode1 = document.registration.pincode;
    var pan1 = document.registration.pan;
...
{
    if (age1 < 0 || age1 > 99 || isNaN(age1)) {
        alert('Age must be between 0 and 2.');
...
{
    const passwordPattern = /^?=.[a-z]+?=.[A-Z]+?=.\d+?=.[\W_].{10}$/;
    if (!passwordPattern.match(password1)) {
        alert('Password must be exactly 10 characters long and include 1 capital letter, 1 number, 1 special character, and small letters.');
                return false;
    }
    else{
